        ...
        return (
            <div className={styles.chartPlaceholder}>
                ...
                <div className={styles.chartImage}>
                    <img src="/images/price-chart.png" alt="Egg Price Trend Chart" />
                </div>{' '}
                <div className={styles.chartLegend}>
                    <div className={styles.legendItem}>
                        <span className={styles.legendColor} style={{ backgroundColor: '#4A6FDC' }}>
                            {' '}
                        </span>{' '}
                        ...
                        <div className={styles.priceComparison}>
                            ...
                            <div className={styles.priceMap}>
                                <div className={styles.mapContainer}>
                                    <img src="/images/us-map.png" alt="US Regional Price Map" />
                                </div>
                                    ...
                                    {regionalPrices.map((region) => (
                                        <div key={region.name} className={styles.regionCard}>
                                            ...
                                            <div className={`${styles.regionChange} ${region.change > 0 ? styles.priceIncrease : styles.priceDecrease}`}>
                                                <i className={`fas fa-arrow-${region.change > 0 ? 'up' : 'down'}`}> </i> {Math.abs(region.change)} %
                                            </div>{' '}
                                        </div>
                                    ))}{' '}
                                </div>{' '}
                        ...
                        <div className={styles.statsList}>
                            ...
                            {priceData[activeYear].stats.map((stat, index) => (
                                <div key={index} className={styles.statItem}>
                                    <div className={styles.statValue}> {stat.value} </div> <div className={styles.statLabel}> {stat.label} </div>{' '}
                                    {stat.change && (
                                        <div className={`${styles.statChange} ${stat.change > 0 ? styles.positive : styles.negative}`}>
                                            <i className={`fas fa-arrow-${stat.change > 0 ? 'up' : 'down'}`}> </i> {Math.abs(stat.change)} %
                                        </div>
                                    )}{' '}
                                </div>
                            ))}{' '}
                            ...
                            {priceData[activeYear].drivers.map((driver, index) => (
                                <li key={index}>
                                    ...
                                    <div
                                        ...
                                        style={{ backgroundColor: driver.impact === 'high' ? '#EF4444' : driver.impact === 'medium' ? '#F59E0B' : '#10B981' }}
                                    >
                                        {' '}
                                        {driver.impact}{' '}
                                    </div>{' '}

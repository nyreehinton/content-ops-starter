...
interface BloombergMarginChartProps {
  className?: string;
  title?: string;
  subtitle?: string;
  [key: string]: any;
...
const BloombergMarginChart: React.FC<BloombergMarginChartProps> = ({ 
  className,
  title = 'Gross & Operating Margin Trends',
  subtitle,
  ...rest
}) => {
  // Sample data for Margin Trends - from bloomberg1_backup.md
  const data = {
  ...
  // Calculate scales
  const minValue = Math.floor(Math.min(...data.datasets.flatMap(d => d.data)) / 10) * 10; // Round to nearest ten
  const maxValue = Math.ceil(Math.max(...data.datasets.flatMap(d => d.data)) / 10) * 10; // Round to nearest ten
  ...
  const generatePathData = (dataset: Dataset) => {
  ...
  const generateAreaPath = (dataset: Dataset) => {
    ...
    return `${linePath} L ${xScale(data.labels.length - 1)} ${yScale(0)} L ${xScale(0)} ${yScale(0)} Z`;
  };
...
export default BloombergMarginChart; 